...
export default function RegisterPage() {
  ...
  const [alertType, setAlertType] = useState<'info' | 'success' | 'warning' | 'error'>('error');
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAlertMessage('สมัครสมาชิกสำเร็จ! กำลังนำทาง...');
      setShowAlert(true);
    } catch (e: any) {
      setErr(e?.message ?? 'สมัครสมาชิกไม่สำเร็จ');
      
      setAlertType('error');
      setAlertMessage(e?.message ?? 'สมัครสมาชิกไม่สำเร็จ');
      setShowAlert(true);
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="space-y-6">
          <form onSubmit={submit} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg text-sm font-medium text-white bg-[#64D1E3] hover:bg-[#5BC0D3] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#64D1E3] transition-all duration-200 shadow-sm hover:shadow-md"
            >
              Sign Up
            </button>
          </form>
